...
// MARK: - Script Progress Tracker

@MainActor
final class ScriptProgressTracker: ObservableObject {

    // MARK: - Published

    @Published var currentMoodIndex: Int = 0
    @Published var sectionCompletions: [ExpressiveScript.Mood: Double] = [
        .excited: 0,
        .normal: 0,
        .sleepy: 0
    ]
    ...
    var currentMood: ExpressiveScript.Mood {
        ExpressiveScript.Mood.allCases[safe: currentMoodIndex] ?? .excited
    }

    var currentSection: ExpressiveScript.Section? {
        ExpressiveScript.sections[safe: currentMoodIndex]
    }

    var isComplete: Bool {
        currentMoodIndex >= ExpressiveScript.sections.count
    }

    // MARK: - Progress Updates

    func updateProgress(from recognizer: SpeechRecognizer) {
        guard let section = currentSection else { return }

        // Calculate completion for current section
        let completion = recognizer.completionPercentage(for: section.keyPhrases)
        sectionCompletions[section.mood] = completion

        // Check if section is complete (80% threshold)
        if completion >= 0.8 {
    ...
    func advanceToNextSection() {
        if currentMoodIndex < ExpressiveScript.sections.count {
            // Mark current as fully complete
            sectionCompletions[currentMood] = 1.0
            currentMoodIndex += 1
        }
    }
    ...
    func reset() {
        currentMoodIndex = 0
        sectionCompletions = [.excited: 0, .normal: 0, .sleepy: 0]
        overallProgress = 0
    }

    private func updateOverallProgress() {
        let totalCompletion = sectionCompletions.values.reduce(0, +)
        overallProgress = totalCompletion / Double(ExpressiveScript.sections.count)
    }
}
